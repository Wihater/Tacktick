...
HEROS = {0: 'DrowRanger', 1: 'Oracle', 2: 'Pudge', '': '', 3: 'Mars',
         4: 'BountyHunter'}  # Словарь с героями и их индексацией для self.select_char
...
class Fight():
    ...
    def ult_fall(self, who):
        for i in range(3):
            if self.plrs != None:
                self.plrs[i].damage(self.enems[1].ult1)
    ...
    def update(self):
        if self.phase == "startgame":
            ...
            if None not in self.plrs and random.randint(1, 10) > 6 and enemy.name == 'Roshan':
                self.enems[self.getenem - 1].anim('u')
                for i in range(3):
                    self.plrs[i].damage(enemy.ult1)
            elif enemy.level in [1, 2, 3, 6, 8]:
                self.enems[self.getenem - 1].anim('a')
            ...
            for i in self.mapdeffect:
                ...
                if 'hp_regen' in kash:
                    for j in self.plrs:
                        if j != None:
                            j.heal(kash[1])
                if 'mana_regen' in kash and self.mana < 4:
                    self.mana += 1
...
class Dota:
            ...
            for event in pygame.event.get():
                ...
                if self.screen == 1:  # Отдельные функции для разных экранов
                    ...
                    if event.type == pygame.MOUSEBUTTONUP:
                        ...
                        if self.room[0] == 10:  # Кнопки передвижения на карте (прикольна)
                            if 960 < event.pos[0] < 1260 and 800 < event.pos[1] < 850:
                                self.board = Fight(self.select_char, [None, 'Roshan8', None], mapdeffect=self.buffs)
                                self.screen = 4
                                self.boss = True
                    ...
                    if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and \
                        ...
                        if self.boss:
                            self.gold_add(self.board.getgold * 10)
                            self.fin_gold += self.board.getgold * 10
                            self.screen = 'f'
                        else:
                            self.screen = 3
    ...
    def movement(self):  # Перемещение между комнатами
        if self.map[self.room[0] - 1][self.room[1] - 1] == 'event':
            ...
            for i in range(3):
                kash[i] += str((self.room[0] + self.room[1] - 2) // 4 + 1)
            self.board = Fight(self.select_char, kash, mapdeffect=self.buffs)
            self.screen = 4
    ...
    def pick_screen(self):  # Экран составления отряда
        ...
        for i in range(len(self.hero_list)):  # ПНГ персонажей
    ...
    def map_screen(self):  # Окно карты
        ...
        if self.room[0] == 10:  # Кнопки для перемещения яуауаяууф
        ...
        if self.board.plrs != [None, None, None] and self.board.enems != [None, None, None]:
            ...
            for i in range(3):
                if self.board.enems[i] != None:
                    if self.board.enems[i].name == 'Roshan':
                        kash1alpha = pygame.image.load(self.board.enems[i].get_image()).convert_alpha()
                        kash1surf = pygame.transform.scale(kash1alpha, (390, 360))
                        kash1rect = kash1surf.get_rect()
                        screen.blit(kash1surf, (kash1rect.x + 700, kash1rect.y + 400, kash1rect.width - 50,
                                                kash1rect.height - 50))
                    ...
                    pygame.draw.rect(screen, (255, 97, 97), [602 + 200 * i, 473,
                                                             116 * self.board.enems[i].hp /
                                                             self.board.enems[i].standhp, 15])
            self.board.update()
        elif self.board.enems == [None, None, None]:  # После победы\поражения
            backG_Image = pygame.image.load(self.board.get_map())
            backG_rect = backG_Image.get_rect()
            screen.blit(backG_Image, backG_rect)
            ...
            screen.blit(backG_Image, backG_rect)
            if self.boss:
                self.print_text(75, str(self.board.getgold * 10), 'yellow', (410, 420))
            elif self.map[self.room[0] - 1][self.room[1] - 1] == "elite":
    ...
    def boss_screen(self):  # Экран босса (крутой)
        self.fight_screen()

    def event_pick(self):  # Случайный выбор события с условиями
        self.connection = sqlite3.connect('DOTAS.db')
        self.cursor = self.connection.cursor()
    ...
    def event_screen(self):  # Экран события
    ...
    def event_res(self, res):  # Результат события на основе выбора
        if self.event[-1] != 'x':
    ...
    def fin_screen(self):  # Финальный экран
        pygame.draw.rect(screen, (135, 135, 161), (400, 400, 550, 150))
        self.print_text(60, 'Игра окончена!', (82, 65, 191), (490, 100))
        self.print_text(50, f'Вы заработали {self.fin_gold} золота!', 'yellow', (450, 450))
    ...
    def print_text(self, size, toprint, col, rect):  # Крутая функция для печати
        font = pygame.font.SysFont('sistem', size)
        text = font.render(toprint, True, col)
        screen.blit(text, rect)

    def print_event(self, size, toprint, col, rect):  # Не крутая функция для печати (на событиях)
        font = pygame.font.SysFont('sistem', size)
        self.rect = rect
        for i in toprint:
